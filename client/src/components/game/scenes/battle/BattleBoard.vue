<template>
    <g>
        <svg viewBox="0 0 83 75" width="83" height="75" x="48.05" y="64.94">
            ...
            <g class="board">
                <rect class="cls-1" x="2.95" y="2.98" width="24" height="18" />
                <rect class="cls-1" x="29.55" y="44.06" width="24" height="17.96" />
                <rect class="cls-1" x="56.14" y="44.02" width="24" height="18" />
                <rect class="cls-1" x="56.14" y="2.93" width="24" height="18" />
                <rect class="cls-1" x="56.14" y="23.56" width="24" height="18" />
                <rect class="cls-1" x="2.95" y="23.49" width="24" height="18" />
                <rect class="cls-1" x="2.96" y="44.01" width="24" height="18" />
                <rect class="cls-1" x="29.55" y="3.07" width="24" height="18" />
                <rect class="cls-1" x="29.5" y="23.49" width="24" height="18" />
            </g>
            ...
            <g class="board-item" v-for="item in this.boardItems" :key="item.id + '_' + item.type" 
                ...
                <g v-if="item.type === 'virus'">
                    <rect class="cls-1" x="10.87" y="11.91" width="2.35" height="2.35" />
                    <rect class="cls-1" x="17.87" y="11.89" width="2.35" height="2.35" />
                    ...
                    <rect class="cls-1" x="13.2" y="18.65" width="4.37" height="2.31" />
                    <polygon class="cls-1" points="22.26 7.25 10.87 7.25 10.87 7.29 8.76 7.29 8.76 7.29 8.37 7.29 8.37 20.96 8.83 20.96 8.83 18.65 10.87 18.65 10.87 16.34 8.83 16.34 8.83 9.57 10.87 9.57 10.87 7.56 20.22 7.56 20.22 9.6 22.26 9.6 22.26 16.34 20.22 16.34 20.22 18.65 22.26 18.65 22.26 20.96 22.68 20.96 22.68 7.25 22.26 7.25" />
                </g>
                <g class="player" v-if="item.type === 'player' || item.type === 'navi'">
                    <g v-if="item.turnType === 'defense'">
                        ...
                        <rect class="cls-1" x="13.8" y="16.79" width="2.77" height="4.2" />
                        <polygon class="cls-1" points="19.71 3 19.71 2.97 19.28 2.97 18.69 2.97 15.13 2.97 13.8 2.97 13.8 5.52 10.95 5.52 9.39 5.52 9.39 11.13 10.95 11.13 10.95 8.34 13.8 8.34 13.8 6.96 15.13 6.96 15.13 4.05 17.94 4.05 17.94 8.35 17.91 8.35 17.91 8.34 16.57 8.34 16.57 11.12 17.91 11.12 17.91 9.72 18.69 9.72 18.69 9.73 19.31 9.73 19.31 12.54 19.28 12.54 18.69 12.54 16.57 12.54 16.57 15.36 17.93 15.36 17.93 19.66 18.69 19.66 19.28 19.66 19.31 19.66 19.31 20.96 19.94 20.96 19.94 3 19.71 3" />
                        <polygon class="cls-1" points="12.37 9.73 12.37 12.56 9.39 12.56 9.39 13.79 9.39 15.29 9.39 16.84 9.39 19.66 9.39 20.96 11.07 20.96 11.07 19.66 12.37 19.66 12.37 13.79 13.8 13.79 13.8 9.73 12.37 9.73" />
                    </g>
...
import { SceneNames, Events } from '../../../../global/constants.js';
...
export default {
    ...
    mounted() {
        ...
        this.$socket.client.emit('startTurn', this.$store.state.battle.id);
    },
    beforeDestroy() {
    ...
    sockets: {
        startTurnResult(blockedItems, currentTurn) {
            ...
            if(this.$store.state.battle.turnType === "attack") {
                this.createAttackArea(this.getChipArea());
            } else if(this.$store.state.battle.turnType === "defense") {
                this.createPlayer();
            }            
        },
        ...
        fireAttack(isAttackHit, attackPower) {
            if(this.$store.state.battle.turnType === "defense") {
                if(isAttackHit) {
    ...
    methods: {
        ...
        createPlayer() {
            ...
            do {
                ...
                playerItem = {
                    id: `${row}_${column}`,
                    type: 'player',
                    turnType: this.$store.state.battle.turnType,
                    posX: column * 26.6,
                    posY: row * 20.5
                }
            } while (this.boardItems.some(x => x.id === playerItem.id && x.type === 'blocked'));

            this.boardItems.push(playerItem);     
            //Send socket for player
            this.$socket.client.emit('defenseMove', this.$store.state.battle.id, playerItem);       
        },
        moveTo(position) {
            if(this.$store.state.battle.turnType === "attack") {
                    ...
                    for (let i = 0; i < attackAreas.length; i++) {
                        ...
                        if((atkRow >= 0 && atkRow <= 2) && (atkColumn >= 0 && atkColumn <= 2)) {
                            var area = {
                                ...
                                type: 'attack',
                                canShow: this.boardItems.some(x => x.id === `${atkRow}_${atkColumn}` && x.type === 'blocked') === false,
                                posX: atkColumn * 26.6,
                                posY: atkRow * 20.5
                            }
                ...
                var defensePlayer = this.boardItems
                    .find(x => x.type === 'player');

                if(defensePlayer) {
                    var arry = defensePlayer.id.split("_");
                    ...
                    if(!this.boardItems.some(x => x.id === newPosId && x.type === 'blocked') &&
                        (row >= 0 && row <= 2) && (column >= 0 && column <= 2)) {
                            ...
                            this.boardItems.splice(this.boardItems.indexOf(defensePlayer), 1);

                            defensePlayer.id = newPosId;
                            defensePlayer.posX = column * 26.6,
                            defensePlayer.posY = row * 20.5
                            
                            this.boardItems.push(defensePlayer);

                            //Send socket for player
                            this.$socket.client.emit('defenseMove', this.$store.state.battle.id, defensePlayer);   
                        }
                }
            }
        },
        ...
        onConfirmation() {
            if(this.$store.state.battle.turnType === "attack") {
                ...
                var defensePlayer = this.boardItems.find(x => x.type === 'player');
                var isAttackHit = this.boardItems.some(x => x.id === defensePlayer.id && x.type === 'attack');
                var attackPower = 0;
                ...
                if(isAttackHit) {
                    if(this.pluggedChip) {
                        ...
                        attackPower = this.$store.state.session.naviStatus.at
                    }
                }
